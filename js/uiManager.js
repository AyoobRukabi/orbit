class UIManager {
    ...
    hideModal() {
        ...
        // FIX: Reset Editing State so hover works again!
        this.isEditing = false;
        this.editingPlanetName = null;
    }
    ...
    showPlanetInfo(data, onEdit, onManageMoons, onClose) {
        ...
        if (data.isOrbit) {
            // FIX: Make orbit info match planet info
            title = `Orbit of ${data.name}`;
            const i = data.info;
            details = `
                <p><strong>Diameter:</strong> ${i.diameter}</p>
                <p><strong>Distance from Sun:</strong> ${i.distanceFromSun}</p>
                <p><strong>Orbital Period:</strong> ${i.orbitalPeriod}</p>
                <p style="margin-top:10px; font-style:italic; color:#aaa; border-top:1px solid #333; padding-top:10px;">
                    This is the orbital path of ${data.name}.
                </p>
            `;
        } else if (data.isMoon) {
