...
class PlanetManager {
    ...
    // NEW METHOD: Update existing moon
    updateMoon(planetName, moonIndex, moonConfig) {
        const planet = this.getPlanetByName(planetName);
        if (!planet || !planet.moons || !planet.moons[moonIndex]) return false;
        ...
        planet.moons[moonIndex] = updatedMoon;
        return true;
    }
    ...
    removePlanet(planet) {
        const index = this.planets.indexOf(planet);
        if (index > -1) {
            // REMOVED '&& planet.isCustom' CHECK TO ALLOW DELETING ANY PLANET
            this.planets.splice(index, 1);
            return true;
        }
        return false;
