class SolarSystem {
    constructor(container) {
        ...
        // Increase threshold for line hover detection
        this.raycaster.params.Line.threshold = 5; 
        
        this.mouse = new THREE.Vector2();
        this.hoveredObject = null; // Renamed from hoveredPlanet
    ...
    createOrbit(distance, planetData) {
        ...
        // Attach Data for Hovering
        line.userData = { isOrbit: true, planetData: planetData };
        return line;
    }
    ...
    createPlanet(data) {
        ...
        if (data.moons) {
            data.moons.forEach(moonData => {
                ...
                // IMPORTANT: Attach data to moon mesh for Raycaster
                moonMesh.userData = { isMoon: true, ...moonData, parentPlanet: data.name };
                
                moonPivot.add(moonMesh);
                planetGroup.add(moonPivot);
                planetGroup.userData.moons.push({ pivot: moonPivot, data: moonData, mesh: moonMesh });
            });
        }
    ...
    updatePlanets(planets) {
        ...
        planets.forEach(data => {
            ...
            // Pass data to createOrbit
            const orbit = this.createOrbit(data.distance, data);
    ...
    checkHoveredPlanet() {
        ...
        // 1. Gather all checkable objects
        let objectsToCheck = [];
        
        // Planets and Moons
        this.planetMeshes.forEach(g => {
            objectsToCheck.push(g.userData.mesh); // Planet mesh
            if(g.userData.moons) {
                g.userData.moons.forEach(m => {
                    // Moon mesh is child of pivot
                    if(m.pivot.children.length > 0) objectsToCheck.push(m.pivot.children[0]);
                });
            }
        });
        
        // Orbit Lines (only if visible)
        if(this.showOrbits) {
            objectsToCheck = objectsToCheck.concat(this.orbitLines);
        }
        ...
        if (this.hoveredObject) {
             ...
             } else if (this.hoveredObject.type === 'Line') {
                 // Reset orbit color
                 this.hoveredObject.material.color.setHex(0x6464aa);
                 this.hoveredObject.material.opacity = 0.3;
             }
        }
        ...
        if (intersects.length > 0) {
            ...
            // Highlight Object
            if (this.hoveredObject.type === 'Line') {
                this.hoveredObject.material.color.setHex(0xffffff);
                this.hoveredObject.material.opacity = 0.8;
                return { isOrbit: true, ...this.hoveredObject.userData.planetData }; // Return planet info for orbit
            } else {
                // Planet or Moon
                this.hoveredObject.material.emissiveIntensity = 0.5;
                return this.hoveredObject.userData;
            }
        } else {
    ...
    reset() { 
        ...
        // FIX: Restore Orbits Visibility on Reset
        this.showOrbits = true;
    ...
    getClickedPlanet(e) {
        ...
        this.raycaster.setFromCamera(this.mouse, this.camera);
        // We only click planets, not orbits
